...
@extend_schema_view(
    ...
    retrieve=extend_schema(description="Returns a single Course selected by `id`."),
    create=extend_schema(
        description="Create a new Course. Teachers/Admins can create Courses."
    ),
    partial_update=extend_schema(description="Updates an existing Course by `id`."),
    destroy=extend_schema(description="Deletes an existing Course by `id`."),
...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        if (
            self.request.user.role == "USER" and self.request.user.type == "TEACHER"
    ...
    def enrollments(self, request, pk):
        ...
        if request.method == "POST":
            ...
            if student_id is None:
                return Response(
                    {"error": "The student field is required."},
            ...
            try:
                ...
                return Response(
                    {"error": "The specified student does not exist."},
            ...
            passed_courses = Enrollment.objects.filter(
                student=user, overall_grade__gte=3
            )

            if prerequisites.exists():
                if not all(
                    prereq.id in passed_courses.values_list("course_id", flat=True)
    ...
    @action(
        ...
        url_path="enrollments/(?P<student_id>[^/.]+)",
    )
    def manage_enrollment(self, request, pk, student_id):
        ...
        try:
            ...
            return Response(
                {"error": "The specified enrollment does not exist."},
                status=status.HTTP_404_NOT_FOUND,
            )
    ...
    def assignments(self, request, pk):
        ...
        if request.method == "DELETE":
            ...
            course.save()
            return Response(
                CourseSerializer(course).data, status=status.HTTP_204_NO_CONTENT
            )
        ...
        if request.method == "GET":
            ...
            if not teacher:
                return Response(
                    {"error": "This course does not have a teacher assigned."},
                    status=status.HTTP_404_NOT_FOUND,
                )
